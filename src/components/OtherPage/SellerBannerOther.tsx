      ...
      <div className="bg-stone-200 h-full w-full flex justify-center items-center pt-24 pb-12 pr-10 ">
        <div className="pr-16 pt-8">
          ...
          {user?.role == 0 ? (
            !isSubscribed?.sub ? (
              <button
                className="bg-green-600 hover:bg-green-800  rounded-md flex items-center px-[0.4rem] py-1 mx-auto mt-3 text-white text-lg "
              ...
              <button
                className="bg-black hover:bg-neutral-800  rounded-md flex items-center px-[0.4rem] py-1 mx-auto mt-3 text-white text-lg "
                onClick={() => cancelsubscribe.mutate()}
              >
                <BsBookmarkPlus />
        ...
        <div className="text-left w-[29rem]">
          <div className="flex items-end">
            <p className="text-4xl font-bold">
              {sellerInfo?.nickname || '익명의 모쿠모쿠 판매자'}
            </p>
            <p className="text-2xl mx-1">님의 페이지</p>
          </div>
          <p className="text-xl mt-4 h-auto max-h-[5rem] overflow-hidden text-ellipsis whitespace-pre-wrap">
            {sellerInfo?.description || ''}
          </p>
          <p className={titleClass}>전화번호</p>
          <p className={contentClass}>
            {sellerInfo?.phoneNumber || '전화번호를 추가해주세요'}
          </p>
          ...
          <p className={contentClass}>
            {sellerInfo?.companyAddress || '주소를 추가해주세요'}
          </p>
        </div>
      </div>
    </div>
