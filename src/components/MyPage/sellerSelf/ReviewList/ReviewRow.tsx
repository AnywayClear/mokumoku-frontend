...
import { dateToStringDot, dayjsToStringDash } from '@/myFunc';
                    ...
                    <div className='flex justify-between'>
                        <div className='w-full'>
                            ...
                            <Link href={`/product/${review.deal.produce.id}`}>
                                <p className='font-bold text-2xl mr-2 underline underline-offset-4 hover:opacity-70'>{review.deal.produce.name}</p></Link>
                                <p className='text-lg text-gray-600 mr-2'>{review.deal.produce.kg}kg</p>
                            </div>
                        </div>
                        <div className='w-1/5 text-right text-lg text-gray-600'>
                                <p className='my-auto'>{dayjsToStringDash(dayjs(review.createdAt))}</p>
                            </div>
                    </div>
                    <div className="mt-4">
                        <div className="flex items-center mb-2">
                            <p className='text-xl font-bold mr-2'>{review.reviewerNickname}</p>
